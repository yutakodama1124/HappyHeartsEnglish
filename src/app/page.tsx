...
const activities = [
  ...
  {
    title: "他団体連携",
    copy: "教育団体、児童館、地域イベント、学生ボランティア団体と協力し、学校の外にも学びの接点を広げます。子どもたちの年齢、会場、時間に合わせて、無理なく参加できる英語アクティビティを一緒に考えます。",
...
export default function Home() {
  ...
  return (
    <div className="bg-[var(--paper)]">
      ...
      <section className="relative overflow-hidden bg-[var(--blush)] px-6 pb-24 pt-12 md:pb-32 md:pt-16">
        <div className="container-shell">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer(0.07)}>
            ...
            <motion.p variants={fadeUp(0.16)} className="body-xl mt-7 max-w-3xl">
              英語絵本と地域イベントを通して、子どもたちが英語にふれる最初の記憶を、
              もっと楽しく、もっとあたたかいものにします。
            </motion.p>
            <motion.p variants={fadeUp(0.2)} className="body-lg mt-5 max-w-3xl">
            ...
            <motion.div variants={fadeUp(0.22)} className="mt-8 flex flex-wrap gap-3">
              ...
              <Link href="/contact" className="focus-ring rounded-full">
                <Button variant="outline" size="lg" className="bg-white px-7">
                  参加・連携の相談
                </Button>
              </Link>
            </motion.div>
          ...
          <motion.div
            ...
            className="mt-14 grid items-end gap-7 lg:grid-cols-[minmax(0,0.68fr)_minmax(320px,0.32fr)]"
          >
            <SnapshotImage
              ...
              className="aspect-[16/9] min-h-[20rem]"
              tilt="left"
            />
            <SnapshotImage
      ...
      <section className="section-shell bg-[var(--paper)]">
        <div className="container-shell grid gap-10 lg:grid-cols-[0.86fr_1.14fr] lg:items-center">
          <div>
            <CrayonTitle>高校生の近さで、英語の入口をやさしくする。</CrayonTitle>
        ...
        <div className="container-shell">
          ...
          <div className="space-y-14">
            {activities.map((activity, index) => (
              <article key={activity.title} className="grid items-center gap-8 lg:grid-cols-2">
                <SnapshotImage
                  src={activity.image}
                  alt={activity.title}
                  sizes="(max-width: 768px) 100vw, 42vw"
                  className={`aspect-[4/3] ${index % 2 === 1 ? "lg:order-2" : ""}`}
                  imageClassName={activity.image.endsWith(".png") ? "object-contain p-6" : "object-cover"}
                ...
                <div className={index % 2 === 1 ? "lg:order-1" : ""}>
                  ...
                  <p className="body-lg mt-5">{activity.copy}</p>
      ...
      <section className="section-shell bg-[var(--paper)]">
        <div className="container-shell">
          <div className="mb-10 max-w-3xl">
            <CrayonTitle>数字と写真で残す、活動の空気。</CrayonTitle>
          </div>
        ...
        <div className="mask-fade-x mt-14">
          <div className="animate-marquee">
            {[...galleryImages, ...galleryImages].map((image, index) => (
              <figure
                key={`${image.src}-${index}`}
                className="mx-3 w-[76vw] bg-white p-2 shadow-[var(--shadow-card)] md:w-96"
              >
        ...
        <div className="container-shell">
          <div className="mb-10 max-w-3xl">
            ...
            <p className="body-lg mt-6">
              ...
              英語をきっかけにした小さな接点を増やすための活動です。文京区で英語イベントや教育ボランティアを探している方に、活動内容が伝わるように整理しています。
            </p>
          </div>
          <div className="grid gap-5 lg:grid-cols-3">
            {audienceBlocks.map((block) => (
              <article key={block.title} className="surface-card p-7">
                <h3 className="title-h3 text-[var(--ink)]">{block.title}</h3>
                <p className="mt-5 text-base leading-8 text-[var(--ink)]/72">{block.copy}</p>
              </article>
            ))}
          </div>
        </div>
          ...
          <div className="grid gap-4">
            {newsItems.map((item) => (
              <Link
                key={item.title}
                href={item.href}
                className="group rounded-[var(--r-md)] border border-[var(--line-soft)] bg-white p-6 shadow-[var(--shadow-card)] transition-[border-color,transform,box-shadow] duration-300 hover:-translate-y-0.5 hover:border-[var(--line-strong)] hover:shadow-[var(--shadow-lift)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--pink-deep)]"
              >
      ...
      <section className="rounded-t-[var(--r-lg)] bg-[var(--pink)] px-6 py-24 text-white md:py-32">
